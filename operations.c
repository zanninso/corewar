...
/*   Updated: 2019/10/11 02:55:16 by aait-ihi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init_op_tab(t_op *op)
{
	...
	op[10] = (t_op){10, "ldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 25, 1, 0, 2, 1, 1};
	...
	op[14] = (t_op){14, "lldi", 3, {T_REG | T_DIR | T_IND, T_DIR | T_REG, T_REG}, 50, 1, 1, 2, 1, 1};
...
void mem_op(t_process *prc, t_op_component *cmp)
{
	...
	if (cmp->code == 2 || cmp->code == 13)
		prc->reg[cmp->arg[1]] = get_mem(prc->memory, prc->pc + cmp->arg[0], 4);
